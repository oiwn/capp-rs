...
- For backend changes that need services (Redis/Mongo/Postgres), guard with feature flags and document required setup.
- Aim to keep coverage stable (Codecov badge enforced in CI); add tests alongside new behaviors.

## Commit & Pull Request Guidelines
- Follow existing short imperative commit style (`update rustis for queue`, `make clippy happy`); keep changes scoped.
