...
pub struct RedisRoundRobinTaskStorage<D> {
    pub key: String,
    pub redis: rustis::client::Client,
    pub tags: Arc<Mutex<std::collections::HashSet<String>>>, // All the tags
    pub current_tag_index: Arc<AtomicUsize>, // To maintain the round-robin
...
impl<D> RedisRoundRobinTaskStorage<D> {
    ...
    fn get_next_queue(&self) -> Result<String, RedisTaskStorageError> {
        ...
        let len = queues.len();
        if len == 0 {
            return Err(RedisTaskStorageError::EmptyValue(
                "queue_keys".to_string(),
            ));
        }
